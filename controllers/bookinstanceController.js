...
exports.bookinstance_delete_get = function(req, res, next) {
  BookInstance.findById(req.params.id).populate('book').exec(function(err, bookinstance){
    ...
    res.render('bookinstance_delete',{title: bookinstance.book.title, bookinstance: bookinstance});
  })
};

// Handle BookInstance delete on POST.
exports.bookinstance_delete_post = function(req, res) {
    BookInstance.findByIdAndRemove(req.body.instanceid, function(err){
      if(err){return next(err)};
...
exports.bookinstance_update_get = function(req, res, next) {
    async.parallel({
      ...
      if (results.bookinstance == null){
        let  err = new Error('Copy not fornd')
        return next(err);
      }
...
exports.bookinstance_update_post = [
    ...
    body('imprint', 'Imprint must be specific').trim().isLength({min:1}),
    body('due_back', 'Invalid date').optional({checkFalsy: true}).isISO8601(),
    ...
    let bookinstance = new BookInstance({
      ...
      if(!errors.isEmpty()){
        Book.find({},'title').exec(function(err, books){
          if(err){return next(err);}
          res.render('bookinstance_form',{title:'Update copy', bookinstnce: bookinstance, book_list:books})
        })
